...
class Login():
    ...
    def Loginup(self,request):
        ...
        query = models.web_UserInfo.objects.filter(username=username,status=1,useing=1).values()
        try:
            if query.__len__() > 0:
                print(2)
                if query[0]['password'] == password:
                    ...
                    models.web_UserInfo.objects.filter(username=username).update(old_login_time=datetime.datetime.today())
