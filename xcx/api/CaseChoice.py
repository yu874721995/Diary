...
class caseChoice():
    ...
    def addChoice(self, request):
        ...
        query_cp = models.Comcp_mk.objects.filter(name=cpname, status=1, type=1).values()
        # 如果已存在则无法添加
        if len(query_cp) != 0:
        ...
        if types == 1 or types == '1':
            try:
                ...
                models.Comcp_mk.objects.create(**dic)
            ...
            query_mk = models.Comcp_mk.objects.filter(name=cpname, status=1, type=2, subjection=subjection).values()
            if len(query_mk) != 0:
    ...
    def queryForProduct(self, request):
        # 仅查询产品列表
        query = models.Comcp_mk.objects.filter(status=1, type=1).values()
        data = []
        for item in query:
            mores = {}
            mores['id'] = item['id']
    ...
    def queryForOur(self, request):
        querys = models.Comcp_mk.objects.filter(status=1, type=1).values()
        data = []
        for items in querys:
            mores = {}
            mores['name'] = items['name']
            mores['value'] = items['id']
            query = models.Comcp_mk.objects.filter(status=1, type=2, subjection=items['id']).values()
            datas = []
            for item in query:
                more = {}
                more['value'] = item['id']
    ...
    def comList(self, request):
        ...
        com = models.Commodity.objects.all().values().order_by('status', '-create_time')
        count = models.Commodity.objects.all().count()
        data = []
        ...
        for i in com:
            ...
            if num > fy[1]:
                break
            com_data = {}
            com_data['com_id'] = i['id']
            com_data['status'] = i['status']
            com_data['Commodity_name'] = i['Commodity_name']
            com_data['Comodity_introduction'] = i['Comodity_introduction']
            com_data['create_time'] = i['create_time']
            com_data['Comodity_Specifications'] = i['Comodity_Specifications']
            com_data['Commodity_Company'] = i['Commodity_Company']
            com_data['Commodity_money'] = i['Commodity_money']
            com_data['Commodity_details'] = i['Commodity_details']
            mk = models.Comcp_mk.objects.filter(id=i['Comodity_type']).values()[0]
            mk_name = mk['name']
            cp = models.Comcp_mk.objects.filter(id=mk['subjection']).values()[0]['name']
            com_data['subjection_cp'] = cp
            com_data['subjection_mk'] = mk_name
            username = models.web_UserInfo.objects.filter(id=i['user_id']).values()[0]['username']
            com_data['username'] = username
            pic = models.Commodity_pic.objects.filter(Commodity_id=i['id']).values()
            pic_data = []
            for i in pic:
                pic_data.append(i['pic_path'])
            com_data['com_pic'] = pic_data
