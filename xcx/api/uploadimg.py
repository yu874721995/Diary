...
from xcx.models import UploadImage
...
def uploadImage(request):
    ...
    if not file:  # 文件对象不存在， 返回400请求错误
        return HttpResponse(json.dumps({'errno':3,'msg':'没有文件'}))
    # 图片大小限制
    if not pIsAllowedFileSize(file.size):
        return HttpResponse(json.dumps({'errno':2,'msg':'文件太大'}))
    # 计算文件md5
    md5 = pCalculateMd5(file)
    uploadImg = UploadImage.getImageByMd5(md5)
    if uploadImg:   # 图片文件已存在， 直接返回
        return HttpResponse(json.dumps({'errno':0,'msg':'已存在','data': [uploadImg.getImageUrl()]}))
    # 获取扩展类型 并 判断
    ext = pGetFileExtension(file)
    if not pIsAllowedImageType(ext):
        return HttpResponse(json.dumps({'errno':1,'msg':'文件类型错误'}))
    # 检测通过 创建新的image对象
    ...
    try:
        ...
        print(e)
        return HttpResponse(json.dumps({'errno':4,'status':1,"msg":'报错了'}))
    # 返回图片的url以供访问
    return HttpResponse(json.dumps({'errno':0,'status':1,"data": [uploadImg.getImageUrl()],'g_url':uploadImg.getImageBgUrl()}))
